...
| Shadeform | aggregator API | free key | list | Fails gate 1 of the admission rubric as a price input: the key is ours and a reader has none, so no print could be recomputed by a stranger. Usable at most as a validation cross-check | not built (key-gated) | 2026-09-24 |
