...
| Intraday sweeps (all live sources above) | the same endpoints, read again through the day | as above | research only — **never an index input** | `config/intraday.yaml` sets a cadence per source, and this row is the "says otherwise and why" the conduct rule requires. vast.ai, RunPod, Hyperstack's stock feed and gpuhunt are read **hourly**, because a marketplace book and a stock feed move inside a day and one read at 11:00 cannot show it. gpuhunt downloads dstack's published catalog, so that hourly read reaches dstack's bucket and none of the providers. Rate cards and catalog APIs (Azure, Scaleway, Seeweb and the Computable recipes) are read **every six hours**, since reading a price list that changes a few times a year every hour would put 24 times the load on somebody else's server to learn nothing. A source read by the 11:00 run is not read again until its cadence comes round. Reads go to `data/intraday/`, which no print reads | live | 2026-09-26 |
