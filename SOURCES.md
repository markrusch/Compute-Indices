...
| Seeweb pricing page (on-demand + committed rates) | `GET https://www.seeweb.it/en/products/cloud-server-gpu` | none | list | Public, server-rendered pricing page (confirmed no JavaScript required, live fetch 2026-09-15); one GET reads the NVIDIA H100 SXM card's on-demand price plus its published 3-, 6- and 12-month committed rates. **Shadow**: stored under `source=seeweb`, which factors.yaml's panel does not list for the `seeweb` provider (only `static_yaml` is), so no row reaches a print. Duplicates the on-demand figure `static_yaml` already carries by hand from the same page — an independent reading, not a replacement; `config/providers/seeweb.yaml` is untouched | shadow | 2026-09-15 |
