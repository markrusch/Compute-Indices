...
| datacrunch | neocloud | `static_yaml`: H100 |
...
9. EUR companion = USD value ÷ the most recent ECB EUR/USD reference rate dated **on or before** the print date. The ECB publishes ~14:00 UTC, after the 11:00 UTC cut-off, so the EUR leg is T-1 on most days; a run after the ECB publication picks up the same-day rate. The rate a print used is recorded with it and is what `reproduce` converts at.
...
# --------------------------------------------------------------------------------------
# EXPLICIT PANEL — added 2026-09-11, when this file was frozen as the 0.3.0-dev snapshot.
#
# 0.3.0-dev never had an enumerated panel. Its constituent population was "whatever the
# collectors produced", with unlisted providers defaulting to the neocloud segment. That
# was harmless only while the collector set stayed fixed; once new collectors start
# storing rows, the same rule would admit them to prints computed under this version.
#
# The block below is that implicit population made explicit: every (provider, collector,
# class) triple that had produced a qualifying row by 2026-09-11, plus the static entries
# listed under `segments` above. It is a re-expression, not a change. Every print stored
# under 0.3.0-dev recomputes to the same value, provider count and constituent set with it
# in place (`python -m tci.run reproduce`, and tests/test_reproduce.py in CI).
panel:
